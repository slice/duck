...
import Data.Flags qualified as Flags
import Di qualified
...
import DuckBot.Effects.HTTP (HttpEff)
...
readConfig path = do
  result <- eitherDecodeFileStrict' @BotConfig path
  either (fail @IO) pure result
...
interpretEffects di config =
  void
    ...
    . interpretTimeGhc
    . interpretHttp
    . runBotIO' (BotToken $ DuckBot.Config.token config) Flags.allFlags Nothing

runBot :: IO ()
runBot = do
  config <- readConfig "./config.json"
  Di.new $ \di ->
    interpretEffects di config $ do
